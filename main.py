...
async def get_page_data(session, page):
    ...
    async with session.get(url=url, headers=headers) as response:
        ...
        cars_items = soup.find(
            'div', class_='table-view-list').find_all('div', class_='list-item')
        base_url = 'https://www.mashina.kg'
        for ci in cars_items:
            car_link = base_url + ci.find('a').get('href')
            car_data = ci.find('a')
            make_model = car_data.find(
            ...
            engine_cap = car_data.find('div', class_='item-info-wrapper').find(
                'p', class_='year-miles').contents[2].replace(', ', '').strip()
            color = car_data.find(
                'div', class_='item-info-wrapper').find('p', class_='year-miles').find('i')['title']
            body_type = car_data.find('div', class_='item-info-wrapper').find(
                'p', class_='body-type').text.split(', ', 1)[0].strip()
            engine_type = car_data.find('div', class_='item-info-wrapper').find(
                'p', class_='body-type').text.split(', ', 1)[1].strip()
            wheel_pos = car_data.find(
                'div', class_='item-info-wrapper').find('p', class_='volume').text.split(', ')[0].strip()
            city = car_data.find('div', class_='block city').find(
                'p').contents[0].strip()
            try:
                transmission = car_data.find('div', class_='item-info-wrapper').find(
                    'p', class_='year-miles').contents[4].strip().replace(', ', '')
            except:
                transmission = ''
            try:
                mileage = car_data.find(
                    'div', class_='item-info-wrapper').find('p', class_='volume').text.split(', ')[1].strip()
            except:
                mileage = ''
            cars_data.append(
                {
...
async def gather_data():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        pages_count = int(soup.find('ul', class_='pagination').find_all(
            'a')[-1].attrs['data-page'])
...
async def check_notif(df):
    ...
    df.transmission = df.transmission.fillna(
        df.engine_cap).replace('', 'электро')
    df.engine_cap = df.engine_cap.replace('автомат', 2).replace(
    ...
    if len(df_notifications) > 0:
        ...
        df_notif_to_send_2 = df_notif_to_send_1[df_notif_to_send_1['_merge'] == 'left_only'].reset_index(
            drop=True)
        pd.concat([df_notifications, df_notif_sent], ignore_index=True).drop_duplicates(
        ).to_csv('csv_data/sorted_dfs/df_notif_sent.csv', index=False)
...
async def concat_hdata():
    ...
    df.engine_cap = df.engine_cap.replace(
    ...
    df_c = df_c[df_c["engine_cap"].isin(
        multi_engine_cap[multi_engine_cap].index)]
    multi_make_model = df_c["make_model"].value_counts() > 5
    df_c = df_c[df_c["make_model"].isin(
        multi_make_model[multi_make_model].index)]
    df_c.reset_index(drop=True, inplace=True)
    df_group = df_c.groupby(['make_model', 'year']).describe()
    df_group.columns = ['__'.join(col).strip()
                        for col in df_group.columns.values]
    dfn = df_group[['price_usd__count', 'price_usd__mean']]
    dfn = dfn[dfn['price_usd__count'] > 3]
    dfn.reset_index(inplace=True)
    dfn.rename(columns={'price_usd__count': 'count',
               'price_usd__mean': 'mean_price'}, inplace=True)
    ...
    while True:
        ...
        if df_notif_to_send.empty is False:
            for i in range(len(df_notif_to_send)):
                card = f"{hlink(df_notif_to_send['make_model'].iloc[i], df_notif_to_send['car_link'].iloc[i])}\n" \
