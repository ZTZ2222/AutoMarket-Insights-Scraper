...
async def check_notif(df):
    ...
    df.engine_cap = df.engine_cap.replace('автомат', 2).replace('вариатор', 1.5).replace('электро', 1.5)
    ...
    if len(df_notifications) > 0:
        ...
        df_notif_sent = df_notif_sent.loc[:, ~df_notif_sent.columns.str.contains('_merge')]
        df_notif_sent = df_notif_sent.loc[:, ~df_notif_sent.columns.str.contains('^Unnamed')]
        df_notif_to_send_1 = pd.merge(df_notifications,df_notif_sent, how="outer", indicator=True)
        df_notif_to_send_2 = df_notif_to_send_1[df_notif_to_send_1['_merge']=='left_only'].reset_index(drop=True)
        pd.concat([df_notifications,df_notif_sent], ignore_index=True).drop_duplicates().to_csv('csv_data/sorted_dfs/df_notif_sent.csv', index=False)
